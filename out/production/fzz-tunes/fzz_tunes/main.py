...
def add_screen(stdscr):
    ...
    window.addstr(3, 0, "[+] Increase Volume   [-] Decrease Volume [q] Quit                 ",
                  curses.color_pair(3))  # Set color for the line
    while True:
        ...
        total_length = 50
        current_progress = int((progress_seconds / total_duration) * total_length)
...
def downloaded(window):
    window.addstr(50, 50, "", curses.color_pair(1))
...
def listen(window):
    hotkeys = {
        ord('p'): play_n_pause,
        ord('r'): lambda: player.music.rewind(),
        ord('s'): stop_music(window),
        ord('+'): increase_vol,
        ord('-'): decrease_vol,
        ord('q'): exit_player,
...
def stop_music(window):
    return lambda: player.music.stop()
